...
import com.thales.googlehashcode.common.model.gametheory.Move;
 ...
 */
public class TreeSearch<M extends Move<G>, G extends Game> {
    private final com.thales.googlehashcode.common.model.time.Timer timer;
    ...
    static class TreeSearchNode<M, G extends Game> extends TreeNode<M, G> {
     ...
     */
    public TreeSearch(Timer timer, double depthPenaltyFactor, ScoreConverter converter) {
        this.timer = timer;
        this.depthPenaltyFactor = depthPenaltyFactor;
     ...
     */
    public M best(final G game, final MoveGenerator<M, G> generator) {
        toBeExpanded.clear();
     ...
     *        The move generator
     */
    public void prun(M executedMove, MoveGenerator<M, G> generator) {
     ...
     */
    public M continueBest(MoveGenerator<M, G> generator) {
        evaluationsPerformed=0;
        try {
            treeSearchLoop(generator);
        } catch (TimeoutException e) {
    ...
    private List<TreeSearchNode<M, G>> expansion(TreeSearchNode<M, G> toExpand, MoveGenerator<M, G> generator) throws TimeoutException {
    ...
    private void treeSearchLoop(final MoveGenerator<M, G> generator) throws TimeoutException {
